...
class COIL(BaseLearner):
    ...
    def solving_ot_to_old(self):
        ...
        T = torch.tensor(T).float().cuda()
        print(T.is_cuda())
        print(self._network.fc.weight[-current_class_num:, :].is_cuda())
        print(F.normalize(self._network.fc.weight[-current_class_num:, :], p=2, dim=1).is_cuda())
        # T = T.clone().detach()
        transformed_hat_W = torch.mm(
            T.T,
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                if fda == 'standard':
                    ...
                    output = self._network(inputs, phase='train')
                    loss_fre_aug = gama * _fre_aug_loss(output["reconstruct_L"], output["reconstruct_H"], targets_L, targets_H)
                else:
                ...
                if self._old_network is not None:
                    ...
                    if epoch < 1:
                        ...
                        loss = (
                            ...
                            + 0.001 * (weight_ot_init * new_branch_distill_loss) + loss_fre_aug
                        )
                    else:
                        features = F.normalize(output["features"], p=2, dim=1)
                        if i % 30 == 0:
                        ...
                        loss = (
                            ...
                            + clf_loss * (1 - self.lamda)
                            + self.args["reg_term"]
                            * (weight_ot_co_tuning * old_branch_distill_loss) + loss_fre_aug
                        )
                else:
                    loss = clf_loss + loss_fre_aug

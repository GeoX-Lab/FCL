...
class IL2A(BaseLearner):
    ...
    def _train(self, train_loader, test_loader):
        ...
        if self._cur_task in []:
            self._network.load_state_dict(torch.load(
                "{}_{}_{}_{}.pkl".format(self.args["model_name"], self.args["init_cls"], self.args["increment"],
    ...
    def _train_function(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                logits, loss_clf, loss_fkd, loss_proto, loss_fre_aug = self._compute_il2a_loss(inputs, targets)
                loss = loss_clf + loss_fkd + loss_proto + gama*loss_fre_aug
                ...
                if fda == 'standard':
                    loss_fre_aug += loss_fre_aug.item()
            ...
            if epoch % 5 != 0:
                info = 'Task {}, Epoch {}/{} => Loss {:.3f}, Loss_clf {:.3f}, Loss_fkd {:.3f}, Loss_proto {:.3f}, Loss_fre_aug {:.3f}, Train_accy {:.2f}'.format(
                    self._cur_task, epoch + 1, self._epoch_num, losses / len(train_loader),
                                    losses_clf / len(train_loader), losses_fkd / len(train_loader),
                                    losses_proto / len(train_loader), loss_fre_aug / len(train_loader), train_acc)
            else:
                test_acc = self._compute_accuracy(self._network, test_loader)
                info = 'Task {}, Epoch {}/{} => Loss {:.3f}, Loss_clf {:.3f}, Loss_fkd {:.3f}, Loss_proto {:.3f}, Loss_fre_aug {:.3f}, Train_accy {:.2f}, Test_accy {:.2f}'.format(
                    self._cur_task, epoch + 1, self._epoch_num, losses / len(train_loader),
                                    losses_clf / len(train_loader), losses_fkd / len(train_loader),
                                    losses_proto / len(train_loader), loss_fre_aug / len(train_loader), train_acc, test_acc)
    ...
    def _compute_il2a_loss(self, inputs, targets):
        ...
        if self._cur_task == 0:
            return logits, loss_clf, torch.tensor(0.), torch.tensor(0.), loss_fre_aug
        ...
        return logits, loss_clf, loss_fkd, loss_proto, loss_fre_aug

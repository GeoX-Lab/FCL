...
class BEEFISO(BaseLearner):
    ...
    def incremental_train(self, data_manager):
        ...
        if self._cur_task > 0:
            if self.random or self.imbalance:
                val_dset = data_manager.get_finetune_dataset(known_classes=self._known_classes,
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                if fda == 'standard':
                    ...
                    logits = out["logits"]
                    loss_fre_aug = gama * _fre_aug_loss(out["reconstruct_L"], out["reconstruct_H"], targets_L,
                                                        targets_H)
                else:
                ...
                loss = loss + loss_en + loss_fre_aug
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                if fda == 'standard':
                    ...
                    inputs, _, targets_L, targets_H = amplitude_mix_single_batch(inputs, pseudo_targets,
                                                                                 num_classes=self._total_classes - self._known_classes)
                    outputs = self._network(inputs, 'train')
                    loss_fre_aug = gama * _fre_aug_loss(outputs["reconstruct_L"], outputs["reconstruct_H"], targets_L,
                                                        targets_H)
                else:
                    pseudo_targets = targets.clone()
                ...
                for task_id in range(self._cur_task + 1):
                    if task_id == 0:
                        pseudo_targets = torch.where(targets < self.data_manager.get_accumulate_tasksize(task_id),
                                                     task_id, pseudo_targets)
                    elif task_id == self._cur_task:
                        pseudo_targets = torch.where(targets - self._known_classes + 1 > 0,
                                                     targets - self._known_classes + task_id, pseudo_targets)
                    else:
                        pseudo_targets = torch.where((targets < self.data_manager.get_accumulate_tasksize(task_id)) & (
                                    targets > self.data_manager.get_accumulate_tasksize(task_id - 1) - 1), task_id,
                                                     pseudo_targets)
                ...
                loss = loss_clf + loss_fe + loss_en + loss_fre_aug
    ...
    def _fusion(self, train_loader, test_loader, optimizer, scheduler):
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            for i, (_, inputs, targets) in enumerate(train_loader):
                ...
                loss = loss_clf + loss_fe + loss_kd + loss_fre_aug
    ...
    def _construct_exemplar_imbalance(self, data_manager, m, random=False, imbalance=None):
        ...
        if random:
            ...
            selected_exemplars = []
            selected_targets = []
            logging.info("Contructing exmplars, totally random...({} total instances  {} classes)".format(increment * m,
                                                                                                          increment))
            data, targets, idx_dataset = data_manager.get_dataset(np.arange(self._known_classes, self._total_classes),
            ...
            if imbalance is None:
                ...
                logging.info("Constructing exmplars, Imbalance...({} or {} per classes)".format(m - m // imbalance,
                                                                                                (m + m // imbalance)))
            elif imbalance < 1:
                '''
                exp type
            ...
            for class_idx in range(self._known_classes, self._total_classes):
                ...
                self._data_memory = np.concatenate((self._data_memory, selected_exemplars)) if len(
                    self._data_memory) != 0 \
                    else selected_exemplars
    ...
    def _construct_exemplar_unified_imbalance(self, data_manager, m, random, imbalance):
        ...
        if random:
            ...
            for class_idx in range(self._known_classes, self._total_classes):
                data, targets, class_dset = data_manager.get_dataset(np.arange(class_idx, class_idx + 1),
                                                                     source='train',
                                                                     mode='test', ret_data=True)
                class_loader = DataLoader(class_dset, batch_size=self.args["batch_size"], shuffle=False, num_workers=4,
                                          pin_memory=True)
                ...
                exemplar_dset = data_manager.get_dataset([], source='train', mode='test',
                                                         appendent=(selected_exemplars, exemplar_targets))
                exemplar_loader = DataLoader(exemplar_dset, batch_size=self.args["batch_size"], shuffle=False,
                                             num_workers=4)

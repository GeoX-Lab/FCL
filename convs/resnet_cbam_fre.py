...
class ResNet(nn.Module):
    ...
    def forward(self, x, phase='test'):
        ...
        if phase == 'train':
            L_info = self.distangler_L(x)
            x_L_image = self.decoder_L(L_info)
            H_info = self.distangler_H(x)
            x_H_image = self.decoder_H(H_info)

            x = self.conv1x1(L_info) + H_info
...
def resnet18_cbam_fre(pretrained=False, **kwargs):
...
def resnet34_cbam_fre(pretrained=False, **kwargs):
...
def resnet50_cbam_fre(pretrained=False, **kwargs):
...
def resnet101_cbam_fre(pretrained=False, **kwargs):
...
def resnet152_cbam_fre(pretrained=False, **kwargs):

...
from convs.resnet_decoder import *
import torch
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.distangler_H = nn.Sequential(nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1),
                                          nn.BatchNorm2d(256),
                                          nn.ReLU(),
                                          )

        self.distangler_L = nn.Sequential(nn.Conv2d(512, 256, kernel_size=3, stride=1, padding=1),
                                          nn.BatchNorm2d(256),
                                          nn.ReLU(),
                                          )

        self.conv1x1 = conv1x1(256, 256)

        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.out_dim = 256 * block.expansion
    ...
    def _forward_impl_basic(self, x):
    ...
    def forward(self, x, phase='test'):
        out = self._forward_impl_basic(x)

        if phase == 'train':
            fmap_lst = out['fmaps'][-1]

            L_info = self.distangler_L(fmap_lst)
            x_L_image = self.decoder_L(L_info)
            H_info = self.distangler_H(fmap_lst)
            x_H_image = self.decoder_H(H_info)

            features = self.conv1x1(L_info) + H_info
            pooled = self.avgpool(features)  # [bs, 512, 1, 1]
...
def resnet10_fre(pretrained=False, progress=True, **kwargs):
    """
...
def resnet26_fre(pretrained=False, progress=True, **kwargs):
    """
...
def resnet18_fre(pretrained=False, progress=True, **kwargs):
...
def resnet34_fre(pretrained=False, progress=True, **kwargs):
...
def resnet50_fre(pretrained=False, progress=True, **kwargs):
...
def resnet101_fre(pretrained=False, progress=True, **kwargs):
...
def resnet152_fre(pretrained=False, progress=True, **kwargs):
...
def resnext50_32x4d_fre(pretrained=False, progress=True, **kwargs):
...
def resnext101_32x8d_fre(pretrained=False, progress=True, **kwargs):
...
def wide_resnet50_2_fre(pretrained=False, progress=True, **kwargs):
...
def wide_resnet101_2_fre(pretrained=False, progress=True, **kwargs):

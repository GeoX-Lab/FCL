...
#
#         self.conv_end = nn.Conv2d(128,3,kernel_size=3,stride=1,padding=1)
#
#
#         self.tanh = nn.Tanh()
...
#
#
# models = resnet18_decoder()
# torchsummary.summary(models, input_size=(256,14,14))
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, upsample=None, flag=True):
        ...
        self.conv0_1 = nn.Conv2d(inplanes, planes, 3, padding=1)
        self.bn0_1 = nn.BatchNorm2d(planes)

        self.conv1 = conv3x3(planes, planes)
        self.bn1 = nn.BatchNorm2d(planes)
        self.relu = nn.ReLU(inplace=True)
        self.conv2 = Upsample2d(inplanes, planes)
        self.bn2 = nn.BatchNorm2d(inplanes)
        self.upsample = upsample
        self.stride = stride
        self.flag = flag
...
class ResNetDecoder(nn.Module):
    ...
    def __init__(self, block, layers):
        ...
        self.layer0 = self._make_layer(block, 256, layers[0])
        self.layer1 = self._make_layer(block, 128, layers[1])
        self.layer2 = self._make_layer(block, 64, layers[2])
        self.layer3 = self._make_layer(block, 32, layers[3])
        self.layer4 = self._make_layer(block, 16, layers[4])


        self.conv_end = nn.Conv2d(16, 3, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        x = self.layer0(x)  # B × 128 × 14 × 14 -> B × 128 × 28 × 28
        x = self.layer1(x)  # B × 64 × 28 × 28 -> B × 64 × 56 × 56
        x = self.layer2(x)  # B × 32 × 56 × 56 -> B × 32 × 112 × 112
        x = self.layer3(x)  # B × 16 × 112 × 112 -> B × 16 × 224 × 224
        x = self.layer4(x)

        x = self.conv_end(x)  # B × 3 × 224 × 224
        out = self.sigmoid(x)

        return out

def resnet18_decoder(pretrained=False):
    model = ResNetDecoder(BasicBlock, [2, 2, 2, 2, 2])
    return model
